...
class GradientSynthesizer {
  ...
  playGradientTone(color: string) {
    const frequency = GRADIENT_FREQUENCIES[color as keyof typeof GRADIENT_FREQUENCIES] || 440;
    
    console.log(`🎹 Playing synthetic gradient tone for ${color} at ${frequency}Hz`);
